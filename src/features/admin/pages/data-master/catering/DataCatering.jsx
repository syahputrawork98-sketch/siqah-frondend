...
import { fetchAdminCaterings } from "@/features/admin/api/adminApi";
import { DEFAULT_CATERING_LIST } from "@/features/admin/model/adminDataMappers";

export default function DataCatering() {
  const {
    data: fetchedCaterings,
    error,
    isLoading,
    reload,
  } = useAsyncData(fetchAdminCaterings, {
    initialData: DEFAULT_CATERING_LIST,
  });
  const [dataCatering, setDataCatering] = useState(DEFAULT_CATERING_LIST);
  const {
    selectedItem: selectedCatering,
    modalView,
    modalEdit,
    modalDelete,
    modalTambah,
  ...
  useEffect(() => {
    setDataCatering(fetchedCaterings);
  }, [fetchedCaterings]);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {isLoading ? (
        <LoadingState message="Memuat data catering..." />
      ) : error ? (
        <ErrorState
          message={error?.message || "Gagal memuat data catering."}
          onRetry={reload}
        />
      ) : dataCatering.length === 0 ? (
        <EmptyState message="Belum ada data catering." />
      ) : (
